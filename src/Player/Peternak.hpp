...
class Peternak : public Player {
    ...
    public:
        ...
        Peternak(string nama) : Player(nama,"Peternak"){}
        void beriMakan();
        void ternak(CultivatedObject* hewan, int row, int col){
            this->lahan.setBarang(row,col,hewan);
        }
        int hitungKekayaan(){ // Apakah harus ada game ?  upd: game gw ilangin
            int total = 0;
        ...
        int bayarPajak(){ // apakah harus ada game ?
            int KKP = this->hitungKekayaan() - 11;
            float persentase;
            int pajak;
            if(KKP > 0){
                if(KKP <= 6){
